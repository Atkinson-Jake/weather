...
let handleWeatherResponse = function(response) {
  ...
  $("#current-conditions-icon").html(icon(current.icon))
  $("#current-conditions-text").html(Math.round(current.temperature) + '&deg;<br/>' + current.summary)

  for (let i = 0; i<6; i++){
    let forecast = response.daily.data[i]
    html = '<div class="col">'
    html = html + '<h2>' + days[n+i] + '</h2>'
    html = html + '<h3>' + icon(forecast.icon) +'</h3>'
    html = html + '<h4>' + Math.round(forecast.temperatureHigh) + '&deg;|' + Math.round(forecast.temperatureLow) + '&deg;</h4>'
